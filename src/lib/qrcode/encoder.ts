/**
 * QR Code generator for ISO/IEC 18004, pure canvas, zero dependencies.
 *
...
export function encodeToModules(text: string): Matrix {
...
function selectVersion(dataLen: number): number {
    for (let v = 1; v <= 40; v++) {
        ...
        const overhead = v <= 9 ? 2 : 3; // mode/count bytes rounded up
...
function computePenalty(m: Matrix, size: number): number {
    ...
    // Rule 3: Finder-like patterns with a 1:1:3:1:1 ratio.
