...
describe('callScript', () => {
    ...
    it('rejects malformed or unsafe script URLs as configuration errors before fetching', async () => {
        const fetchImpl = vi.fn();

        await expect(callScript('not-a-url', 'history', {}, { fetchImpl })).rejects.toMatchObject({
            code: 'CONFIG_ERROR',
            userMessage: 'The shortener service is not configured correctly.',
        });
        await expect(
            callScript('javascript:alert(1)', 'history', {}, { fetchImpl }),
        ).rejects.toMatchObject({
            code: 'CONFIG_ERROR',
            userMessage: 'The shortener service is not configured correctly.',
        });
    ...
    it('keeps request serialization helpers available through the api facade', () => {
        expect(serializeParams({ keep: 'yes', count: 2, skip: null })).toEqual({
            keep: 'yes',
            count: '2',
        });

        const requestUrl = buildScriptRequestUrl('https://script.example/exec', 'read', {
            name: 'abc',
        });
        expect(requestUrl).toBe('https://script.example/exec?action=read&name=abc');

        expect(normalizeError(new Error('offline'))).toMatchObject({
            code: 'NETWORK_ERROR',
