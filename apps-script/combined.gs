...
var DB_COL = createColumnIndexMap_(DATABASE_HEADERS, {
    TIMESTAMP: 'Timestamp',
    ALIAS: 'Alias',
    LONG_LINK: 'Long link',
    UID: 'uid',
    ...
    CREATOR_IP_HASH: 'creator IP hash',
    COUNTRY: 'country',
    REGION: 'region',
    CITY: 'city',
    TIMEZONE: 'timezone',
});
var ACCESS_COUNT_COLUMN = DB_COL.ACCESS_COUNT + 1;
var LAST_ACCESSED_COLUMN = DB_COL.LAST_ACCESSED + 1;
var REPORT_HEADERS = [
    'Timestamp',
    'Alias',
    'Reason',
...
var REPORT_COL = createColumnIndexMap_(REPORT_HEADERS, {
    TIMESTAMP: 'Timestamp',
    ALIAS: 'Alias',
    REASON: 'Reason',
...
var BLOCKED_URL_PATTERNS = [/^javascript:/i, /^data:/i, /^vbscript:/i, /^file:/i, /^ftp:/i];
...
function doGet(e) {
    ...
    try {
        switch (String(action).toLowerCase()) {
            case 'write':
                return handleWrite_(event);
            case 'bulk':
                return handleBulk_(event);
            case 'report':
                return handleReport_(event);
            case 'preview':
                return handlePreview_(event);
            case 'history':
                return handleHistory_(event);
            case 'read':
            default:
                return handleRead_(event);
        }
    } catch (err) {
        return respond_(
            failure_('INTERNAL_ERROR', 'The shortener service failed. Try again shortly.'),
        );
    }
}
...
function handleRead_(e) {
    ...
    var link = findLinkRowByAlias_(sheet, alias);
    if (link) {
        var storedUrl = String(link.row[DB_COL.LONG_LINK] || '').trim();
        var expiry = String(link.row[DB_COL.EXP] || '').trim();
        if (isExpired_(expiry)) {
            return respond_(failure_('LINK_EXPIRED', 'This link has expired.', { url: BASE_URL }));
        }

        var storedHash = String(link.row[DB_COL.PASSWORD_HASH] || '').trim();
        if (storedHash) {
            if (!password) {
                return respond_(
                    failure_('PASSWORD_REQUIRED', 'Password required.', { needsPassword: true }),
                );
            }
            ...
            if (!verifyPassword_(password, storedHash)) {
                return respond_(
                    failure_('WRONG_PASSWORD', 'Incorrect password.', { needsPassword: true }),
                );
            }
        }

        incrementAccessCounter_(sheet, link.rowNumber, link.row);
...
function handlePreview_(e) {
    ...
    if (link) {
        var storedUrl = String(link.row[DB_COL.LONG_LINK] || '').trim();
        var expiry = String(link.row[DB_COL.EXP] || '').trim();
        var hasPassword = !!String(link.row[DB_COL.PASSWORD_HASH] || '').trim();
...
function handleWrite_(e) {
    ...
    if (!rl.allowed) {
        return respond_(
            failure_('RATE_LIMITED', 'Rate limit exceeded. Try again in ' + rl.retryAfter + 's.'),
        );
    }
    ...
    if (expiryParam) {
        ...
        if (isNaN(expiryDate.getTime())) {
            return respond_(
                failure_('INVALID_EXPIRY', 'Invalid expiry date format. Use ISO 8601.'),
            );
        }
    ...
    try {
        ...
        if (!sheet) {
            return respond_(
                failure_(
                    ...
                    'Database unavailable. Ensure a "database" sheet tab exists.',
                ),
            );
        }
        ensureDatabaseHeaders_(sheet);

        var data = sheet.getLastRow() > 0 ? getDatabaseValues_(sheet) : [];
        ...
        if (!passwordParam && !expiryParam) {
            ...
            if (reusable) {
                var metadataStored = updateNetworkMetadata_(
                    sheet,
                    reusable.rowNumber,
                    reusable.row,
                    network,
                ...
                return respond_({
                    ok: true,
                    shortUrl: BASE_URL + reusable.alias,
                    reused: true,
        ...
        if (rawAlias) {
            ...
            if (!alias) {
                return respond_(
                    failure_(
                        'INVALID_ALIAS',
                        'Invalid alias. Use 2-64 alphanumeric characters, hyphens, or underscores.',
                    ),
                );
            }
...
function handleBulk_(e) {
    ...
    if (urls.length > MAX_BULK_URLS) {
        return respond_(
            failure_('TOO_MANY_URLS', 'Maximum ' + MAX_BULK_URLS + ' URLs per bulk request.'),
        );
    }
    ...
    try {
        ...
        var reusableRows = buildReusableRowMap_(data, requestedUrls);
        ...
        for (var preparedIndex = 0; preparedIndex < preparedUrls.length; preparedIndex++) {
            ...
            var reusable = reusableRows[targetUrl];
            if (reusable) {
                if (reusable.rowNumber > 0) {
                    updateNetworkMetadata_(sheet, reusable.rowNumber, reusable.row, network);
                }
                results.push({
                    url: targetUrl,
                    ok: true,
            ...
            if (!alias) {
                results.push({
                    url: targetUrl,
                    ok: false,
                    code: 'ALIAS_GENERATION_FAILED',
                    error: 'Could not generate alias.',
                });
                continue;
            }
            ...
            data.push(row);
            rows.push(row);
            reusableRows[targetUrl] = { alias: alias, row: row, rowNumber: 0 };
...
function handleReport_(e) {
    ...
    var destination = sanitizeReportDestination_(
        getParam_(e, 'destination') || getParam_(e, 'url'),
    );
    var network = collectNetworkContext_(e);
...
function handleHistory_(e) {
    ...
    var data = sheet.getRange(1, 1, sheet.getLastRow(), 12).getValues();
    var links = [];

    for (var i = 0; i < data.length; i++) {
        var rowUid = String(data[i][DB_COL.UID] || '').trim();
        if (rowUid !== uid) continue;

        var alias = normalizeStoredAlias_(data[i][DB_COL.ALIAS]);
        var longUrl = String(data[i][DB_COL.LONG_LINK] || '').trim();
        var created = String(data[i][DB_COL.TIMESTAMP] || '').trim();
        var expiry = String(data[i][DB_COL.EXP] || '').trim();
        var hasPassword = !!String(data[i][DB_COL.PASSWORD_HASH] || '').trim();
...
function normalizeEvent_(e) {
    ...
    if (body) {
        ...
        for (var parsedKey in parsed) {
            if (
                Object.prototype.hasOwnProperty.call(parsed, parsedKey) &&
                isSafeParamKey_(parsedKey)
            ) {
...
function createColumnIndexMap_(headers, names) {
    var map = {};
    for (var key in names) {
        if (Object.prototype.hasOwnProperty.call(names, key)) {
            map[key] = headers.indexOf(names[key]);
        }
    }
    return map;
}

function getDatabaseValues_(sheet) {
    return sheet.getRange(1, 1, sheet.getLastRow(), DATABASE_HEADERS.length).getValues();
}

function findLinkRowByAlias_(sheet, alias) {
    var lastRow = sheet.getLastRow();
    if (lastRow < 1) return null;

    var aliasValues = sheet.getRange(1, 2, lastRow, 1).getValues();
    for (var i = 0; i < aliasValues.length; i++) {
        if (normalizeStoredAlias_(aliasValues[i][0]) !== alias) continue;

        var rowNumber = i + 1;
        var row = sheet.getRange(rowNumber, 1, 1, DATABASE_HEADERS.length).getValues()[0] || [];
        var storedUrl = String(row[DB_COL.LONG_LINK] || '').trim();
        if (validateUrl_(storedUrl)) continue;

        return {
            alias: normalizeStoredAlias_(row[DB_COL.ALIAS]),
            row: row,
            rowNumber: rowNumber,
        };
    }
...
function validateUrl_(url) {
    ...
    if (url.length > MAX_URL_LENGTH)
        return { code: 'URL_TOO_LONG', message: 'URL exceeds maximum length.' };
    if (hasControlChars_(url))
        return { code: 'URL_CONTROL_CHARS', message: 'URL contains invalid characters.' };
    if (!/^https?:\/\//i.test(url))
        return { code: 'URL_PROTOCOL', message: 'URL must start with http:// or https://.' };
    ...
    if (!authority)
        return { code: 'URL_INVALID', message: 'Enter a complete URL with a valid host.' };
    if (authority.indexOf('@') !== -1)
        return { code: 'URL_CREDENTIALS', message: 'URL must not contain embedded credentials.' };
    var parsedAuthority = parseAuthority_(authority);
    if (!parsedAuthority.host)
        return { code: 'URL_INVALID', message: 'Enter a complete URL with a valid host.' };
    if (parsedAuthority.invalidPort)
        return { code: 'URL_INVALID', message: 'Enter a complete URL with a valid port.' };

    var domain = extractDomain_(url);
    if (isBlockedDomain_(domain))
        return { code: 'URL_BLOCKED_DOMAIN', message: 'This domain is blocked.' };
...
function buildReusableUrlMap_(data, requestedUrls) {
    var rows = buildReusableRowMap_(data, requestedUrls);
    var urls = {};
    for (var storedUrl in rows) {
        if (Object.prototype.hasOwnProperty.call(rows, storedUrl)) {
            urls[storedUrl] = rows[storedUrl].alias;
        }
    }
    return urls;
}

function buildReusableRowMap_(data, requestedUrls) {
    var rows = {};
    for (var i = 0; i < data.length; i++) {
        var storedUrl = String(data[i][DB_COL.LONG_LINK] || '').trim();
        if (requestedUrls && !requestedUrls[storedUrl]) continue;
        if (!isReusableRow_(data[i])) continue;
        if (!rows[storedUrl]) {
            rows[storedUrl] = {
                alias: normalizeStoredAlias_(data[i][DB_COL.ALIAS]),
                row: data[i],
                rowNumber: i + 1,
            };
        }
...
function isReusableRow_(row) {
    var alias = normalizeStoredAlias_(row[DB_COL.ALIAS]);
    var storedUrl = String(row[DB_COL.LONG_LINK] || '').trim();
    var expiry = String(row[DB_COL.EXP] || '').trim();
    var passwordHash = String(row[DB_COL.PASSWORD_HASH] || '').trim();
...
function collectNetworkContext_(e) {
    ...
    var region =
        sanitizeNetworkField_(getParam_(e, 'region') || fallback.region) || city || country;
    return {
        ip: ip,
        ...
        region: region,
        city: city,
        timezone: sanitizeNetworkField_(
            getParam_(e, 'tz') || getParam_(e, 'timezone') || fallback.tz || fallback.timezone,
        ),
    };
}
...
function incrementAccessCounter_(sheet, rowNumber, row) {
    try {
        ...
        sheet
            .getRange(rowNumber, ACCESS_COUNT_COLUMN, 1, columnCount)
            .setValues([[current + 1, new Date().toISOString()]]);
    } catch (_) {
...
function isDatabaseHeaderRow_(headers) {
    return (
        String(headers[0] || '').toLowerCase() === 'timestamp' &&
...
function appendDatabaseRow_(sheet, row) {
    ...
    sheet
        .getRange(sheet.getLastRow() + 1, 1, 1, DATABASE_HEADERS.length)
        .setValues([normalizedRow]);
}
...
function updateNetworkMetadata_(sheet, rowNumber, row, network) {
    ...
    var values = [
        chooseMetadataValue_(row[DB_COL.CREATOR_IP], network.ip),
        chooseMetadataValue_(row[DB_COL.CREATOR_IP_HASH], network.ipHash),
        chooseMetadataValue_(row[DB_COL.COUNTRY], network.country),
        chooseMetadataValue_(row[DB_COL.REGION], network.region),
        chooseMetadataValue_(row[DB_COL.CITY], network.city),
        chooseMetadataValue_(row[DB_COL.TIMEZONE], network.timezone),
    ];

    if (
        values.join('|') ===
        [
            String(row[DB_COL.CREATOR_IP] || ''),
            String(row[DB_COL.CREATOR_IP_HASH] || ''),
            String(row[DB_COL.COUNTRY] || ''),
            String(row[DB_COL.REGION] || ''),
            String(row[DB_COL.CITY] || ''),
            String(row[DB_COL.TIMEZONE] || ''),
        ].join('|')
    ) {
        return true;
    }

    sheet.getRange(rowNumber, DB_COL.CREATOR_IP + 1, 1, values.length).setValues([values]);
    return true;
}
...
function hasNetworkMetadata_(network) {
    return !!(
        network &&
        (network.ip || network.country || network.region || network.city || network.timezone)
    );
}
...
function checkRateLimit_(scope, identity, maxRequests) {
    ...
    var key =
        'rl_' +
        sanitizeField_(scope || 'default', 24) +
        '_' +
        sha256Hex_(identity || 'global').slice(0, 24);
