...
interface CombinedScriptContext {
    ...
    findReusableRowForUrl_(
        data: unknown[][],
        targetUrl: string,
    ): { alias: string; row: unknown[]; rowNumber: number } | null;
...
describe('Google Apps Script URL validation', () => {
    it('defines database row access through named schema columns', () => {
        const source = readProjectFile('apps-script/combined.gs');
        const script = loadScript<CombinedScriptContext>('apps-script/combined.gs');

        expect(script.DB_COL.ALIAS).toBe(script.DATABASE_HEADERS.indexOf('Alias'));
        expect(script.DB_COL.LONG_LINK).toBe(script.DATABASE_HEADERS.indexOf('Long link'));
        expect(script.DB_COL.EXP).toBe(script.DATABASE_HEADERS.indexOf('exp'));
        expect(script.DB_COL.PASSWORD_HASH).toBe(script.DATABASE_HEADERS.indexOf('password hash'));
        expect(script.DB_COL.CREATOR_IP).toBe(script.DATABASE_HEADERS.indexOf('creator IP'));
        expect(source).not.toMatch(/\brow\[(?:1|2|10|11|14|15|16|17|18|19)\]/);
        expect(source).not.toMatch(/\bdata\[i\]\[(?:1|2|3|10|11)\]/);
    });
    ...
    it('increments access counters only when a link read succeeds', () => {
        ...
        script.getSheet_ = () => ({
            ...
            getRange: (row: number, column: number, _rowCount: number, _columnCount: number) => ({
                getValues: () =>
                    rows
                        .slice(row - 1, row - 1 + _rowCount)
                        .map((sourceRow) => sourceRow.slice(column - 1, column - 1 + _columnCount)),
    ...
    it('avoids full-sheet reads on the single-alias redirect path', () => {
        const script = loadScript<CombinedScriptContext>('apps-script/combined.gs');
        const rows = [
            [
                'Timestamp',
        ...
        script.getSheet_ = () => ({
            ...
            getDataRange: () => ({
                getValues: () => {
                    fullDataRangeReads += 1;
                    cellsRead += rows.length * rows[0].length;
                    return rows;
                },
            }),
            getRange: (row: number, column: number, rowCount: number, columnCount: number) => ({
                getValues: () => {
                    cellsRead += rowCount * columnCount;
                    return rows
        ...
        expect(script.handleRead_({ parameter: { name: 'target' } })).toMatchObject({
            ok: true,
            url: 'https://example.com/249',
        });
        expect(fullDataRangeReads).toBe(0);
        expect(cellsRead).toBe(rows.length + 20);
    });
    ...
    it('reuses bulk duplicate rows without rescanning existing data per URL', () => {
        const script = loadScript<CombinedScriptContext>(
            'apps-script/combined.gs',
            createAppsScriptGlobals(),
        );
        const originalFindReusableRow = script.findReusableRowForUrl_;
        let repeatedScans = 0;
        const rows = [
            [
                'Timestamp',
        ...
        script.findReusableRowForUrl_ = (data: unknown[][], targetUrl: string) => {
            repeatedScans += 1;
            return originalFindReusableRow(data, targetUrl);
        };
        script.respond_ = (payload: unknown) => payload;
        script.getSheet_ = () => ({
            getLastRow: () => rows.length,
            ...
            results: [
                {
                    ...
                    ok: true,
                    shortUrl: 'https://natsumeaoii.github.io/surl/first',
                    reused: true,
                },
                {
                    url: 'https://example.com/second',
                    ok: true,
                    shortUrl: 'https://natsumeaoii.github.io/surl/second',
                    reused: true,
                },
            ],
        });
        expect(repeatedScans).toBe(0);
    });
    ...
    it('keeps legacy split Apps Script files from defining deployment entry points', () => {
        const getScript = readProjectFile('apps-script/get.gs');
        const postScript = readProjectFile('apps-script/post.gs');

        expect(getScript).not.toMatch(/\bfunction\s+do(?:Get|Post)\s*\(/);
        expect(postScript).not.toMatch(/\bfunction\s+do(?:Get|Post)\s*\(/);
        expect(getScript).toContain('Deploy apps-script/combined.gs instead.');
        expect(postScript).toContain('Deploy apps-script/combined.gs instead.');
    });
});
