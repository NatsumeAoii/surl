...
describe('GitHub Pages routing configuration', () => {
    ...
    it('serves short-link fallback routes locally during development', () => {
        const configSource = readProjectFile('vite.config.ts');
    ...
    it('loads a cache-busted dev entry and disables PWA caching during development', () => {
        const index = readProjectFile('index.html');
        const entry = readProjectFile('src/main.tsx');
    ...
    it('validates the deploy artifact and deployed Pages output in CI', () => {
        ...
        expect(deployValidator).toContain("'.nojekyll'");
        ...
        const verifier = readProjectFile('scripts/verify-deployed-pages.mjs');
        expect(verifier).toContain('matchAll');
        expect(verifier).toContain('type=["\']module["\']');
        expect(verifier).toContain('assets/');
    ...
    it('keeps Apps Script short-link responses on the current production URL', () => {
        const scripts = [
            readProjectFile('apps-script/combined.gs'),
            readProjectFile('apps-script/get.gs'),
            readProjectFile('apps-script/post.gs'),
        ].join('\n');
